...
# 1. DANH SÁCH MÃ CỔ PHIẾU (15 mã Bán lẻ - Tiêu dùng - Thực phẩm - Dược phẩm)
STOCK_CODES = ["MWG", "FRT", "DGW", "PNJ", "MSN", "VNM", "SAB", "SBT", "QNS", "DBC", "VHC", "ANV", "DHG", "IMP", "TRA"]

STOCK_PATTERN = re.compile(r"\b(" + "|".join(STOCK_CODES) + r")\b")

# 2. TỪ KHÓA ĐỊNH DANH DOANH NGHIỆP (Dùng để quét xem bài báo có nhắc đến công ty không)
...
# 3. THÔNG TIN DOANH NGHIỆP (Lưu vào sheet COMPANY_INFO)
...
# 4. DANH SÁCH NGUỒN TIN (Đồng bộ 100% với danh sách RSS_SOURCES và API_SOURCES bên dưới)
SOURCE_INFO: list[dict] = [
    ...
    {"name": "Tuổi Trẻ", "domain": "tuoitre.vn", "categories": "Kinh tế", "type": "RSS", "rss": "Có", "articles": "10k+", "note": "Tin kinh tế, bán lẻ hàng ngày", "has_rss": True, "has_api": False},
...
# 5. DANH SÁCH RSS (Lọc từ nguồn chuẩn, tập trung Kinh doanh, Doanh nghiệp, Thị trường, Tiêu dùng)
RSS_SOURCES: list[dict] = [
    ...
    {"name": "Tuổi Trẻ KD", "url": "https://tuoitre.vn/rss/kinh-te.rss", "cat": "KD"},
...
# 7. TỪ KHÓA TÌM KIẾM THEO MÃ (Cho DDGS)
...
# 8. TỪ KHÓA TÌM KIẾM THEO NGÀNH (Sentiment)
INDUSTRY_SEARCH = [
...
for code, terms in SEARCH_TERMS.items():
    for term in terms:
        ...
        CONFIG_KEYWORDS_DATA.append({
            ...
            "event_type_suggestion": "earnings, expansion, contraction, stock_mention",
            "priority": "High",
...
for i, term in enumerate(INDUSTRY_SEARCH):
    ...
    CONFIG_KEYWORDS_DATA.append({
        ...
        "priority": "Medium",
        "note": "Tìm kiếm theo từ khóa ngành",
    })
...
XLXS_PATH = r"E:\Do-An-2\data\K4_CRAWL_NHOM_4_BAN_LE_TIEU_DUNG.xlsx"
